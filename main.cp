//
//  main.cpp
//  Rocket Launch by Sweta Patil and Vinayak Jaiwant
//
//  Created by Vinayak Jaiwant on 05/06/20.
//  Copyright © 2020 Vinayak Jaiwant. All rights reserved.
//
...
void stars(){
    ...
    for(float s3=5.0;s3<=100; s3+=0.6){
        for(float s4=-6.0;s4<=6;s4+=0.6){
            glPushMatrix();
            glBegin(GL_POINTS);
            glVertex3f(s4,s3,0);
            glEnd();
...
void display()
{
    ...
    stroke_output(0.0, 2.0, "Rocket Launch");
    stroke_output(0.0, -0.2, "By Sweta Patil and Vinayak Jaiwant");
    stroke_output(0.0, -2.0, "Press P to Launch");
